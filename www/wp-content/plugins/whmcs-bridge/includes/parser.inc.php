...
function cc_whmcs_bridge_parser($buffer=null,$current=false) {
    ...
    if (is_object($page_title) && isset($page_title->plaintext)) {
        ...
        $_SESSION['bridgeCurLang'] = $language;
        ...
        if (isset($_REQUEST['catid']))
            $s_url .= 'c'.$_REQUEST['catid'];
        if (isset($_REQUEST['rp']))
            $s_url .= 'rp'.$_REQUEST['rp'];
    ...
    if ($parse) {
        cc_whmcs_log(0, '[' . $ref . '] Parser parsing non-pretty links ('.print_r(array_keys($loop), true).').');
        ...
        foreach ($loop as $rep_url) {
            ...
            $f[] = "/.post\(\'".preg_quote($rep_url,'/')."([a-zA-Z0-9]*?).php.(.*?)/";
...
//            $f[]="/src=\"".preg_quote($rep_url,'/')."assets\/img\/([a-zA-Z0-9\_\.\-]*?).gif/";
//            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'assets/img/$1.gif'.$pid;
//
//            $f[]="/src=\"".preg_quote($rep_url,'/')."assets\/img\/([a-zA-Z0-9\_\.\-]*?).png/";
//            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'assets/img/$1.png'.$pid;
//
//            $f[]="/src=\"".preg_quote($rep_url,'/')."assets\/img\/([a-zA-Z0-9\_\.\-]*?).jpg/";
//            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'assets/img/$1.jpg'.$pid;

//            $f[]="/href=\"".preg_quote($rep_url, '/')."assets\/css\/([a-zA-Z0-9\_\.\-]*?).css/";
//            $r[]="href=\"{$home}?ccce=js&ajax=1&js=".'assets/css/$1.css'.$pid;
//
//            $f[]="/src=\"".preg_quote($rep_url, '/')."assets\/js\/([a-zA-Z0-9\_\.\-]*?).js/";
//            $r[]="src=\"{$home}?ccce=js&ajax=1&js=".'assets/js/$1.js'.$pid;
        ...
        foreach ($loop as $rep_url) {
            if (substr($rep_url, -1) == '/') {
                ...
                $buffer = str_replace('href="' . $rep_url, 'href="' . $home, $buffer);
                $buffer = str_replace('action="' . $rep_url, 'href="' . $home, $buffer);
            } else {
                $buffer = str_replace('$.post(\'' . $rep_url .'/', '$.post(\'' . $home, $buffer);
                $buffer = str_replace('href="' . $rep_url . '/', 'href="' . $home, $buffer);
                $buffer = str_replace('action="' . $rep_url . '/', 'href="' . $home, $buffer);
...
//        $r[] = "href=\"{$home}?ccce=js&ajax=1&js=" . 'assets/$1' . $pid;
    ...
    // whmcsBaseUrl
    $buffer = str_replace("whmcsBaseUrl = ", "whmcsBaseUrl = \"\"; //", $buffer);
...
//    // cvv img etc.
//    $buffer = str_replace("<img src='/assets/", "<img src='".cc_whmcs_bridge_url()."/assets/", $buffer);
//    $buffer = str_replace('<img src="/assets/', '<img src="'.cc_whmcs_bridge_url().'/assets/', $buffer);
...
function cc_whmcs_bridge_parse_url($redir) {
    ...
    if (strstr($redir, 'knowledgebase.php') !== false) {
        $redir = str_replace($folder, '', $redir);
        if (get_option('cc_whmcs_bridge_permalinks'))
            $return = $home.'?ccce='.str_replace('.php?', '&', $redir);
        else
